...
namespace QueryParsing
{
    public sealed class Profiler
    {
        ...
        {
            var lines = extent.EndLineNumber;
    ...
    public sealed class InstrumentAst<TItem> : ICustomAstVisitor where TItem : class 
    {
        internal Profiler Profiler { get; set; }
        internal Expression<Func<TItem, bool>> Builder { get; set; }

        public Expression<Func<TItem, bool>> Translate(Expression<Func<TItem, bool>> predicate, Ast ast)
        {
            if (ast.GetType() != typeof (ScriptBlockAst)) return predicate;

            VisitScriptBlock((ScriptBlockAst) ast);
            return predicate.And(Builder);
        }
        ...
        public StatementAst[] VisitStatements(ReadOnlyCollection<StatementAst> statements)
        {
            ...
            {
                var instrument = (statement is PipelineBaseAst);
                var extent = statement.Extent;
                if (instrument)
                {
